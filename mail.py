...
for chat, info in INFO.items():
    if chat == "folder":
        continue
    try:
        with MailBox(config.SERVER).login(
            info["login"], info["password"], initial_folder="Inbox"
        ) as mailbox:
            ...
            for msg in mailbox.fetch(AND(date_gte=week_ago), bulk=True):
                ...
                if len(attachments) == 0:
                    ...
                    if len(splitted_text) > 1:
                        ...
                        BOT.send_message(chat, splitted_text[0], parse_mode="MARKDOWN")
                else:
                    splitted_text = telebot.util.smart_split(
